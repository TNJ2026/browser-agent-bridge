...
| 操作 | Playwright 风格 locator **加上**对感知节点的按 `ref` 操作（`locator.*Ref`），以及坐标式 `computer.*`（canvas/非 DOM） | 手动 `Input.*` / `Runtime.*` 调用 | locator + 自动等待 |
| 认证态抓取 | 直接读取用户已登录页面 —— 真实 session（含 httpOnly cookie）已就绪 | 自己重建登录态：登录脚本或注入 cookie | 通过登录或 `storageState` 重建登录态 |
| 强 CSP 与响应体 | 按 origin 临时绕 CSP（有 TTL），注入读取/脚本能跑；`network.read` / `network.getResponseBody` 抓 XHR/JSON 响应体 | 自己驱动 Network/Page 域 | context 级绕 CSP；`route`/response API |
| 动作反馈 | 每个动作返回 `whatChanged` 增量（URL、弹窗、焦点、可选 a11y diff）供 agent 循环 | 自己订阅协议事件并关联 | 自动等待 + 自己写断言 |
